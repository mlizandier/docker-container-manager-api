...
export class ContainerAuthService {
	...
	constructor(authorizedContainerRepository?: AuthorizedContainerRepository) {
		this.authorizedContainerRepository =
			authorizedContainerRepository || new AuthorizedContainerRepository();
	}

	public async isAuthorized(name: string): Promise<boolean> {
		try {
			const container =
				await this.authorizedContainerRepository.findByName(name);
